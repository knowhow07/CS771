...
class CustomConv2DFunction(Function):
    ...
    def forward(ctx, input_feats, weight, bias, stride=1, padding=0):
        ...
        # Get dimensions for the output feature map
        N, C_i, H, W = input_feats.size()
        C_o, _, K, K = weight.size()

        # Output height and width calculation
        out_height = (H + 2 * padding - K) // stride + 1
        out_width = (W + 2 * padding - K) // stride + 1

        # Unfold the input feature map into [N, C_i*K*K, L], L is the total number of patches
        input_unfold = unfold(input_feats, kernel_size=K, padding = padding, stride=stride)
        
        # Unfolde the weight to C_0, C_i*K*K
        weight_unfold  = weight.view(C_o, -1)
        
        # Perform matrix multiplication between the unfolded input and the filters
        output_unfold = torch.matmul(weight_unfold, input_unfold)
        
        # Reshape the output to the correct shape [N, C_o, out_height, out_width]
        output = output_unfold.view(N, C_o, out_height, out_width)

        # Add bias if provided
        if bias is not None:
            output = output.add(bias.view(1, bias.size(0), 1, 1))
    ...
    def backward(ctx, grad_output):
        ...
        #################################################################################
        # Fill in the code here
        #### Step 1: Compute the gradient of weights
        Co, Ci, K, K = weight.size()
        # unfold and transpos on the input, size [N, L, Ci*K*K]
        input_unfold_ts = input_unfold.permute(0, 2, 1)  # (batch_size, height * width, in_channels * kernel_height * kernel_width)
        
        # unfold the output gradients to [N, Co, L]
        grad_output_unfold = unfold(grad_output, kernel_size=(1,1))
        #print("input", input_unfold_ts.size(), "output", grad_output_unfold.size())
        
        # Compute gradient of weight by matrix multiplication, size[N, Co, Ci*K*K  ]
        grad_weight_unfold = torch.matmul(grad_output_unfold, input_unfold_ts)
        # print("weight",  grad_weight_unfold.size())
        
        # sum over the batch and fold the gradient of weight
        grad_weight = grad_weight_unfold.sum(dim=0).view(weight.size())
        
        #### Step 2: Compute the gradient of inputs
        # Unfold the weight to [Ci*K*K, Co]
        weight_unfold_ts  = weight.view(Co, -1).transpose(0, 1)
        # print(weight_unfold_ts)
        
        #Compute gradient of weight by matrix multiplication, size[N, (Ci*K*K), L] 
        grad_input_unfold = torch.matmul(weight_unfold_ts, grad_output_unfold)
        # fold the grad input to [N, Ci, H, W]
        grad_input = fold(grad_input_unfold, output_size=(input_height, input_width), kernel_size=kernel_size, 
